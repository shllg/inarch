// The inarch Claude Code skill, bundled as a string so `inarch init` can write it into a
// consumer repo's .claude/skills/inarch/SKILL.md (no network fetch, version-locked to the
// installed inarch). This is the single source of truth for the skill text; inarch's own
...
description: This repo is indexed by graft/. For ANY task here, whether
  ...
  from inarch before grepping or reading source files.
---
...
### 1 · \`inarch ask "<question>" --source\`: locate + understand (the default)
...
### 2 · \`inarch grep "<pattern>"\`: exhaustive find
...
- Search a **short symbol name or literal**, not a full guessed signature: an
  ...
  and signature, keep the bare name) and retry \`inarch grep\` — do NOT switch to
  raw \`grep -rn\`, which is slower and unranked.
- \`-i\` case-insensitive; \`--in <path>\` scopes to a subtree. Raw \`grep -rn\` is
  only for files inarch genuinely doesn't index (docs, configs, brand-new files).

### 3 · \`inarch skeleton <file>\`: a file's API at a glance
...
### 4 · \`inarch callers <symbol>\`: the exact edges
...
### 5 · \`inarch map\`: orientation for an unfamiliar repo or area
...
### 6 · Lifecycle: \`inarch build\` / \`inarch check\`
...
|---|---|---|
| Onboarding / "explain this codebase" | \`inarch map\`, then read the named hub cards | 1 |
| Understanding a flow ("how does X work") | \`inarch ask "<flow>" --source\` | 1 |
| Finding where a change belongs | \`inarch ask "where is <behavior>" --source\` | 1 |
| Editing a symbol you can already name | \`inarch grep "<symbol>"\`, edit at the \`file:line\` (skip \`ask\` — you know where it is) | 1 |
| Renaming / deleting / changing a signature | \`inarch callers <sym> --depth 2\` first | 1 |
| Refactor / multi-file change (before editing) | \`inarch callers <sym> --depth all\` — map every connected file, don't stop at the first | 1 |
| "What does this depend on?" | \`inarch callers <sym> --direction out\` | 1 |
| Finding every occurrence of a pattern | \`inarch grep "<literal>"\` | 1 |
| "What's the API of this file?" | \`inarch skeleton <file>\` | 1 |
| Debugging a failure in area X | \`inarch ask "<symptom>" --source\`, then \`callers\` on the suspect | 1–2 |
| Judging a diff's risk before merge | \`inarch callers <changed sym> --depth 2\` | 1 / symbol |
| Working inside one repo of a monorepo | add \`--in <scope>/\` to ask / grep / callers | n/a |

In a multi-repo workspace, inarch ranks fairly so the biggest repo can't drown
the rest, and every hit carries a \`[scope/]\` label naming its sub-project; when
you already know where you're working, narrow with \`inarch ask "<task>" --in <scope>/\`.
...
- When the task already names the file or symbol to change, go straight there:
  \`inarch grep "<symbol>"\` for the exact \`file:line\`, then edit. Reserve
  \`inarch ask\` for when you don't yet know where the code lives — an \`ask\`
...
- If inarch names a path that isn't on disk, its index is ahead of your checkout
  (a branch switch or unpulled move). Don't read the missing file — \`inarch grep\`
  the symbol to find where it lives now, or run \`inarch build\` to refresh.

## Report what inarch saved, every turn
...
Whenever you used any inarch tool in a turn, close your reply with a one-line
tally summing those numbers across every inarch call you made, e.g.
\`🌱 inarch saved ~12,400 tokens this turn (3 calls)\`. A call with no such line
...
that total alongside the tokens, e.g. \`🌱 inarch saved ~12,400 tokens (~$0.04)
this turn\`. When a line carries no dollar figure, report tokens alone rather
than pricing them yourself.

**Never pipe a inarch command through \`head\`, \`tail\`, or \`sed -n\`.** Every tool
...
- A node lacks a detail: ask a more specific question; only then read source at
  the exact \`file:line\`, never a whole file to rebuild understanding inarch gives.
...
When the inarch MCP server is connected, these are exposed as tools too:
