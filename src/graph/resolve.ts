    ...
    if (e.relation === "contains" && e.targetId) {
      ...
      if (e.specifier) {
        ...
        if (candidates.length === 1) {
          ...
          const hit = resolveThroughReexports(targetFile, e.name, reexports, byId, perFileName, null);
...
function resolveThroughReexports(
  ...
  for (const entry of entries) {
    ...
    const target = resolveImport(entry.specifier, file, byId);
    ...
    const deeper = resolveThroughReexports(target, inner, reexports, byId, perFileName, kinds, seen, hops + 1);
